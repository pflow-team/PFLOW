...
PetscErrorCode PSReadPSSERawData(PS ps,const char netfile[])
{
  ...
  */
  ...
  if(fgets(line,MAXLINE,fp) != NULL){
    ...
    if(fgets(line,MAXLINE,fp) == NULL)  PetscFunctionReturn(0);// for commas
    if(fgets(line,MAXLINE,fp) == NULL)  PetscFunctionReturn(0);// for commas
    dataformatflag++;
  }else{
    PetscFunctionReturn(0);
  ...
  dataformatflag = -1;
  fp = fopen(netfile,"r");
  if(fgets(line,MAXLINE,fp) != NULL){//for case identification
    if(fgets(line,MAXLINE,fp) == NULL)  PetscFunctionReturn(0);// for commas
    if(fgets(line,MAXLINE,fp) == NULL)  PetscFunctionReturn(0);// for commas
    dataformatflag++;
  }else{
...
PetscErrorCode PSSetUp(PS ps)
{
  ...
  */
  ...
  PetscInt Ngbuses=Nbuses,Nglines=Nlines;
  ...
  /* Get the local number of gens and loads */
    ...
    /* Copy line data component to PSLINE */
    for(i=eStart; i < eEnd; i++) {
